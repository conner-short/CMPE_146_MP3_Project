...
bool I2C2::initSlave(unsigned int speedInKhz, void* memBase, uint32_t memLenBytes) {
    /* Handle peripheral power-up, clocking, pins, and interrupt registration */
    if(!init(speedInKhz))
    {
        return false;
    }

    setSlaveRegParams((uint8_t*)memBase, memLenBytes);

    /* Set the slave address as 0x10 */
    LPC_I2C2->I2ADR0 = 0x10;
