...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
        ...
        receivedOwnReadAddress       = 0xA8,
        masterBecameSlaveTx          = 0xB0,
        dataAckedByMaster            = 0xB8,
        dataNackedByMaster           = 0xC0,
        lastDataAckedByMaster        = 0xC8,

        // Slave Receiver States:
        receivedOwnWriteAddress          = 0x60,
        masterBecameSlaveRx              = 0x68,
        receivedGeneralCall              = 0x70,
        masterBecameSlaveRxGenCall       = 0x78,
        receivedDataByteAndAcked         = 0x80,
        receivedDataByteAndNacked        = 0x88,
        receivedDataByteAndAckedGenCall  = 0x90,
        receivedDataByteAndNackedGenCall = 0x98,
        receivedStopOrRepeatedStart      = 0xA0,
    };
    ...
    switch (mpI2CRegs->I2STAT)
    {
        ...
        /* Slave general states */

        case dataNackedByMaster:               // no break
        case lastDataAckedByMaster:            // no break
        case receivedDataByteAndNacked:        // no break
        case receivedDataByteAndNackedGenCall: // no break
        case receivedStopOrRepeatedStart:
            setAckFlag(); /* Wait for address */
            clearSIFlag();
            break;

        /* Slave Tx states */

        case masterBecameSlaveTx:    // no break
        case receivedOwnReadAddress:
            mRegOffset = 0;
        ...
        case receivedGeneralCall:        // no break
        case masterBecameSlaveRx:        // no break
        case masterBecameSlaveRxGenCall: // no break
        case receivedOwnWriteAddress:
            mReceivedTransactionReg = false; /* Reset the transaction base register */
            setAckFlag();                    /* ACK the next data byte */
            clearSIFlag();
            break;

        case receivedDataByteAndAckedGenCall: // no break
        case receivedDataByteAndAcked:
            if(!mReceivedTransactionReg)
            {
                /* Save the base register for this operation */
                mTransactionReg = (uint8_t)mpI2CRegs->I2DAT;
                mReceivedTransactionReg = true;

                mRegOffset = 0; /* Set register index to the beginning */
            }
            else
            {
                /* Check bounds of write operation */
                if((mTransactionReg + mRegOffset) < mRegLenBytes)
                {
                    mRegBase[mTransactionReg + mRegOffset] = (uint8_t)mpI2CRegs->I2DAT;
                    mRegOffset++;
                }
            }
...
void I2C_Base::setSlaveRegParams(uint8_t* regBase, uint32_t lenBytes) {
    mRegBase = regBase;
    mRegLenBytes = lenBytes;
}

void I2C_Base::doReadOp() {
    /* Check read bounds */
    if((mTransactionReg + mRegOffset) < (mRegLenBytes - 1))
    {
        /* Set read byte */
        mpI2CRegs->I2DAT = (uint32_t)mRegBase[mTransactionReg + mRegOffset];

        mpI2CRegs->I2CONSET = 0x04; /* ACK and continue read operation */
    }
    else
    {
        if((mTransactionReg + mRegOffset) < mRegLenBytes) {
        ...
        mpI2CRegs->I2CONCLR = 0x04; /* ACK and stop read operation */
    }
